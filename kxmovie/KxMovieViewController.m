...
-(void) play
{
    ...
    if (_decoder.validAudio) {
        [self enableAudio:YES];     // 点击play的时候 enableAudio
    }
...
- (void) enableAudio: (BOOL) on
{
    ...
    if (on && _decoder.validAudio) {
        ...
        // 设置audioManager的block audioUnit 或者 audioQueue 从外层获取数据， 填充outData
...
- (BOOL) addFrames: (NSArray *)frames
{
    if (_decoder.validVideo) {
        ...
        // _videoFrames 可存可取 读取的线程也可以访问 所以需要 @synchronized
